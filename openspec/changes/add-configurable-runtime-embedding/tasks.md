...
- [x] Update embedding provider resolution so `mode=api` uses configured provider/base URL/model and `timeout_seconds`, while missing API credentials still falls back only through documented environment resolution (verification: unit - config/scorer resolver tests assert selected base URL, model, and timeout without exposing secrets).
...
- [x] Make runtime embedding timeout use `embedding.timeout_seconds` and log expected timeout fallback as a concise warning instead of a full exception stack trace (verification: unit - caplog test confirms timeout returns default fallback and warning text does not include traceback).
- [x] Add compatibility checks that surface mismatch between classifier bundle embedding metadata and runtime embedding backend/model/dimension before silent learned-classifier use (verification: unit - bundle mismatch tests cover model identity and dimension mismatch paths).
...
- [x] Update `README.md` and `config.yaml` examples to document `embedding.mode`, `timeout_seconds`, API provider/model selection, local mode constraints, and disabled mode behavior (verification: manual - docs examples match `EmbeddingConfig` fields and no secrets are introduced).
