...
- Classifier assets in `models/` are detected and reported with clear status: active, unused legacy, missing, or unsupported.
...
- Existing `models/feature_classifier.pkl` is reported with whether the current runtime uses it or not, based on actual code configuration.
...
- Add diagnostics helpers in `src/kani/cli.py` or a small new module under `src/kani/` if needed.
- Add tests under `tests/` that call the CLI via Click's testing utilities and assert masked output and expected warning/error lines.
